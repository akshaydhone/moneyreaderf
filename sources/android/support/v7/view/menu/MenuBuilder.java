...
public class MenuBuilder implements SupportMenu {
    ...
    private void setShortcutsVisibleInner(boolean shortcutsVisible) {
        boolean z = true;
        if (!(shortcutsVisible && this.mResources.getConfiguration().keyboard != 1 && this.mResources.getBoolean(C0183R.bool.abc_config_showMenuShortcutsWhenKeyboardPresent))) {
            z = false;
        }
    ...
    void findItemsWithShortcutForKey(List<MenuItemImpl> items, int keyCode, KeyEvent event) {
        ...
        if (event.getKeyData(possibleChars) || keyCode == 67) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (((SupportMenu.SUPPORTED_MODIFIERS_MASK & modifierState) == (SupportMenu.SUPPORTED_MODIFIERS_MASK & (qwerty ? item.getAlphabeticModifiers() : item.getNumericModifiers()))) && shortcutChar != '\u0000' && ((shortcutChar == possibleChars.meta[0] || shortcutChar == possibleChars.meta[2] || (qwerty && shortcutChar == '\b' && keyCode == 67)) && item.isEnabled())) {
                    items.add(item);
                }
            }
        }
