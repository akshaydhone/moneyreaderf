...
public class LinearLayoutCompat extends ViewGroup {
    ...
    public static class LayoutParams extends MarginLayoutParams {
        ...
        public LayoutParams(Context c, AttributeSet attrs) {
            super(c, attrs);
            this.gravity = -1;
            TypedArray a = c.obtainStyledAttributes(attrs, C0183R.styleable.LinearLayoutCompat_Layout);
            this.weight = a.getFloat(C0183R.styleable.LinearLayoutCompat_Layout_android_layout_weight, 0.0f);
            this.gravity = a.getInt(C0183R.styleable.LinearLayoutCompat_Layout_android_layout_gravity, -1);
            a.recycle();
        }
    ...
    public LinearLayoutCompat(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TintTypedArray a = TintTypedArray.obtainStyledAttributes(context, attrs, C0183R.styleable.LinearLayoutCompat, defStyleAttr, 0);
        int index = a.getInt(C0183R.styleable.LinearLayoutCompat_android_orientation, -1);
        if (index >= 0) {
            setOrientation(index);
        }
        index = a.getInt(C0183R.styleable.LinearLayoutCompat_android_gravity, -1);
        if (index >= 0) {
            setGravity(index);
        }
        boolean baselineAligned = a.getBoolean(C0183R.styleable.LinearLayoutCompat_android_baselineAligned, true);
        if (!baselineAligned) {
            setBaselineAligned(baselineAligned);
        }
        this.mWeightSum = a.getFloat(C0183R.styleable.LinearLayoutCompat_android_weightSum, -1.0f);
        this.mBaselineAlignedChildIndex = a.getInt(C0183R.styleable.LinearLayoutCompat_android_baselineAlignedChildIndex, -1);
        this.mUseLargestChild = a.getBoolean(C0183R.styleable.LinearLayoutCompat_measureWithLargestChild, false);
        setDividerDrawable(a.getDrawable(C0183R.styleable.LinearLayoutCompat_divider));
        this.mShowDividers = a.getInt(C0183R.styleable.LinearLayoutCompat_showDividers, 0);
        this.mDividerPadding = a.getDimensionPixelSize(C0183R.styleable.LinearLayoutCompat_dividerPadding, 0);
        a.recycle();
    }
                ...
                if (child.getVisibility() != 8) {
                    ...
                    if (childExtra > 0.0f) {
                        ...
                        if (lp.height == 0 && heightMode == 1073741824) {
                            ...
                            child.measure(childWidthMeasureSpec, MeasureSpec.makeMeasureSpec(share, ErrorDialogData.SUPPRESSED));
                        } else {
                            childHeight = child.getMeasuredHeight() + share;
                            if (childHeight < 0) {
                                childHeight = 0;
                            }
                            child.measure(childWidthMeasureSpec, MeasureSpec.makeMeasureSpec(childHeight, ErrorDialogData.SUPPRESSED));
                        }
            ...
            if (useLargestChild && heightMode != 1073741824) {
                for (i = 0; i < count; i++) {
                    ...
                    if (!(child == null || child.getVisibility() == 8 || ((LayoutParams) child.getLayoutParams()).weight <= 0.0f)) {
                        child.measure(MeasureSpec.makeMeasureSpec(child.getMeasuredWidth(), ErrorDialogData.SUPPRESSED), MeasureSpec.makeMeasureSpec(largestChildHeight, ErrorDialogData.SUPPRESSED));
                    }
                }
            }
        }
    ...
    void measureHorizontal(int widthMeasureSpec, int heightMeasureSpec) {
                ...
                if (!(child == null || child.getVisibility() == 8)) {
                    ...
                    if (childExtra > 0.0f) {
                        ...
                        if (lp.width == 0 && widthMode == 1073741824) {
                            ...
                            child.measure(MeasureSpec.makeMeasureSpec(share, ErrorDialogData.SUPPRESSED), childHeightMeasureSpec);
                        } else {
                            childWidth = child.getMeasuredWidth() + share;
                            if (childWidth < 0) {
                                childWidth = 0;
                            }
                            child.measure(MeasureSpec.makeMeasureSpec(childWidth, ErrorDialogData.SUPPRESSED), childHeightMeasureSpec);
                        }
                        childState = View.combineMeasuredStates(childState, child.getMeasuredState() & ViewCompat.MEASURED_STATE_MASK);
                    }
                    if (isExactly) {
        ...
        setMeasuredDimension((ViewCompat.MEASURED_STATE_MASK & childState) | widthSizeAndState, View.resolveSizeAndState(Math.max(maxHeight + (getPaddingTop() + getPaddingBottom()), getSuggestedMinimumHeight()), heightMeasureSpec, childState << 16));
        if (matchHeight) {
