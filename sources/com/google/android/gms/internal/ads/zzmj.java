package com.google.android.gms.internal.ads;

import android.os.Bundle;
import com.google.android.gms.dynamic.IObjectWrapper;

public final class zzmj extends zzkt {
    private zzkh zzxs;

    public final void destroy() {
    }

    public final String getAdUnitId() {
        return null;
    }

    public final String getMediationAdapterClassName() {
        return null;
    }

    public final zzlo getVideoController() {
        return null;
    }

    public final boolean isLoading() {
        return false;
    }

    public final boolean isReady() {
        return false;
    }

    public final void pause() {
    }

    public final void resume() {
    }

    public final void setImmersiveMode(boolean z) {
    }

    public final void setManualImpressionsEnabled(boolean z) {
    }

    public final void setUserId(String str) {
    }

    public final void showInterstitial() {
    }

    public final void stopLoading() {
    }

    public final void zza(zzaaw zzaaw) {
    }

    public final void zza(zzabc zzabc, String str) {
    }

    public final void zza(zzahe zzahe) {
    }

    public final void zza(zzjn zzjn) {
    }

    public final void zza(zzke zzke) {
    }

    public final void zza(zzkh zzkh) {
        this.zzxs = zzkh;
    }

    public final void zza(zzkx zzkx) {
    }

    public final void zza(zzla zzla) {
    }

    public final void zza(zzlg zzlg) {
    }

    public final void zza(zzlu zzlu) {
    }

    public final void zza(zzmu zzmu) {
    }

    public final void zza(zzod zzod) {
    }

    public final boolean zzb(zzjj zzjj) {
        zzane.m27e("This app is using a lightweight version of the Google Mobile Ads SDK that requires the latest Google Play services to be installed, but Google Play services is either missing or out of date.");
        zzamu.zzsy.post(new zzmk(this));
        return false;
    }

    public final Bundle zzba() {
        return new Bundle();
    }

    public final IObjectWrapper zzbj() {
        return null;
    }

    public final zzjn zzbk() {
        return null;
    }

    public final void zzbm() {
    }

    public final zzla zzbw() {
        return null;
    }

    public final zzkh zzbx() {
        return null;
    }

    public final String zzck() {
        return null;
    }
}
