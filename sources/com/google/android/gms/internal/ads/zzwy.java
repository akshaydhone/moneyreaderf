...
public final class zzwy {
    ...
    public zzwy(JSONObject jSONObject) throws JSONException {
        if (zzane.isLoggable(2)) {
            ...
            zzakb.m29v(valueOf.length() != 0 ? str.concat(valueOf) : new String(str));
        }
        JSONArray jSONArray = jSONObject.getJSONArray("ad_networks");
        List arrayList = new ArrayList(jSONArray.length());
        int i = -1;
        for (int i2 = 0; i2 < jSONArray.length(); i2++) {
            zzwx zzwx = new zzwx(jSONArray.getJSONObject(i2));
            if (zzwx.zzmf()) {
                this.zzbte = true;
            }
            arrayList.add(zzwx);
        ...
        this.zzbsy = jSONObject.optLong("timeout_ms", -1);
        JSONObject optJSONObject = jSONObject.optJSONObject("settings");
        if (optJSONObject != null) {
            this.zzbsl = optJSONObject.optLong("ad_network_timeout_millis", -1);
            zzbv.zzfd();
            ...
            this.zzbsu = optLong > 0 ? optLong * 1000 : -1;
