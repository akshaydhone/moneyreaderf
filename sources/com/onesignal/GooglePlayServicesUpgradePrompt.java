...
class GooglePlayServicesUpgradePrompt {
    ...
    static class C02721 implements Runnable {
        ...
        public void run() {
            ...
            if (activity != null && !OneSignal.mInitBuilder.mDisableGmsMissingPrompt) {
                ...
                }).setNegativeButton(alertButtonSkip, new C02701()).setNeutralButton(OSUtils.getResourceString(activity, "onesignal_gms_missing_alert_button_close", "Close"), null).create().show();
            }
        }
    }
    ...
    static boolean isGMSInstalledAndEnabled() {
        try {
            return OneSignal.appContext.getPackageManager().getPackageInfo("com.google.android.gms", 1).applicationInfo.enabled;
    ...
    private static boolean isGooglePlayStoreInstalled() {
        try {
            ...
            String label = (String) pm.getPackageInfo("com.google.android.gms", 1).applicationInfo.loadLabel(pm);
    ...
    static void ShowUpdateGPSDialog() {
        if (!isGMSInstalledAndEnabled() && isGooglePlayStoreInstalled() && !OneSignalPrefs.getBool(OneSignalPrefs.PREFS_ONESIGNAL, "GT_DO_NOT_SHOW_MISSING_GPS", false)) {
            OSUtils.runOnMainUIThread(new C02721());
        }
    }
    ...
    private static void OpenPlayStoreToApp(Activity activity) {
        try {
            ...
            apiAvailability.getErrorResolutionPendingIntent(activity, apiAvailability.isGooglePlayServicesAvailable(OneSignal.appContext), PLAY_SERVICES_RESOLUTION_REQUEST).send();
