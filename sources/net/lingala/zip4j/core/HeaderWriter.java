    ...
    public int writeLocalFileHeader(ZipModel zipModel, LocalFileHeader localFileHeader, OutputStream outputStream) throws ZipException {
        ...
        try {
            ...
            if (Zip4jUtil.isStringNotNullAndNotEmpty(zipModel.getFileNameCharset())) {
                copyByteArrayToArrayList(localFileHeader.getFileName().getBytes(zipModel.getFileNameCharset()), byteArrayList);
            } else {
    ...
    public void updateLocalFileHeader(LocalFileHeader localFileHeader, long offset, int toUpdate, ZipModel zipModel, byte[] bytesToWrite, int noOfDisk, SplitOutputStream outputStream) throws ZipException {
        ...
        try {
            ...
            if (noOfDisk != outputStream.getCurrSplitFileCounter()) {
                ...
                String fileName = new StringBuffer(String.valueOf(parentFile)).append(System.getProperty("file.separator")).toString();
                if (noOfDisk < 9) {
                    fileName = new StringBuffer(String.valueOf(fileName)).append(fileNameWithoutExt).append(".z0").append(noOfDisk + 1).toString();
                } else {
                    fileName = new StringBuffer(String.valueOf(fileName)).append(fileNameWithoutExt).append(".z").append(noOfDisk + 1).toString();
                }
                closeFlag = true;
