...
public class SplitOutputStream extends OutputStream {
    ...
    public SplitOutputStream(File file, long splitLength) throws FileNotFoundException, ZipException {
        if (splitLength < 0 || splitLength >= PlaybackStateCompat.ACTION_PREPARE_FROM_SEARCH) {
    ...
    private void startNextSplitFile() throws IOException {
        try {
            ...
            String parentPath = this.outFile.getParent() == null ? "" : new StringBuffer(String.valueOf(this.outFile.getParent())).append(System.getProperty("file.separator")).toString();
            if (this.currSplitFileCounter < 9) {
                currSplitFile = new File(new StringBuffer(String.valueOf(parentPath)).append(zipFileWithoutExt).append(".z0").append(this.currSplitFileCounter + 1).toString());
            } else {
                currSplitFile = new File(new StringBuffer(String.valueOf(parentPath)).append(zipFileWithoutExt).append(".z").append(this.currSplitFileCounter + 1).toString());
            }
            this.raf.close();
    ...
    public boolean isBuffSizeFitForCurrSplitFile(int bufferSize) throws ZipException {
        ...
        } else if (this.splitLength < PlaybackStateCompat.ACTION_PREPARE_FROM_SEARCH || this.bytesWrittenForThisPart + ((long) bufferSize) <= this.splitLength) {
            return true;
        } else {
            return false;
        }
