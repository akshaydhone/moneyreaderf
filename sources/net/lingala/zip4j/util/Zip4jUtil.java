                ...
                for (int i = 0; i <= numberOfThisDisk; i++) {
                    if (i == numberOfThisDisk) {
                        ...
                        if (zipFileName.indexOf(".") >= 0) {
                            partFile = currZipFile.substring(0, currZipFile.lastIndexOf("."));
                        } else {
                            partFile = currZipFile;
                        }
                        retList.add(new StringBuffer(String.valueOf(partFile)).append(fileExt).append(i + 1).toString());
                    }
                }
                return retList;
            } else {
        ...
        if (isStringNotNullAndNotEmpty(file)) {
            ...
            if (isStringNotNullAndNotEmpty(rootFolderPath)) {
                ...
                if (tmpFile.isDirectory()) {
                    tmpFileName = new StringBuffer(String.valueOf(tmpFileName.replaceAll("\\\\", InternalZipConstants.ZIP_FILE_SEPARATOR))).append(InternalZipConstants.ZIP_FILE_SEPARATOR).toString();
                } else {
                    tmpFileName = new StringBuffer(String.valueOf(tmpFileName.substring(0, tmpFileName.lastIndexOf(tmpFile.getName())).replaceAll("\\\\", InternalZipConstants.ZIP_FILE_SEPARATOR))).append(tmpFile.getName()).toString();
                }
                fileName = tmpFileName;
            } else {
                File relFile = new File(file);
                if (relFile.isDirectory()) {
                    fileName = new StringBuffer(String.valueOf(relFile.getName())).append(InternalZipConstants.ZIP_FILE_SEPARATOR).toString();
                } else {
